...
def get_possible_moves():
    return (
        ...
        "- Чтобы узнать ID другого пользователя, пересылайте его сообщение"
    )
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'help':
        ...
        text = "Чтобы узнать ID другого пользователя, перешлите сообщение от него."
    ...
    if context.user_data.get('awaiting_other_id'):
        ...
        if fwd_user:
            ...
            await update.message.reply_text(
                ...
                f"Его язык: {fwd_user.language_code}",
                reply_markup=main_keyboard()
            )
        else:
            await update.message.reply_text(
